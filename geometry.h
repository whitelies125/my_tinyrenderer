...
template <typename T> struct vec<4,T> {
    ...
    template <class U> vec(const vec<4,U> &v);
          T& operator[](const size_t i)       { assert(i<3); return i<=0 ? x : (1==i ? y : (2==i ? z : w)); }
    const T& operator[](const size_t i) const { assert(i<3); return i<=0 ? x : (1==i ? y : (2==i ? z : w)); }
